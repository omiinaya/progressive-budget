...
self.addEventListener("install", (event) => {
  ...
  event.waitUntil(
    caches
      .open(CACHE_CONFIG.STATIC_CACHE)
      .then((cache) => {
...
self.addEventListener("fetch", (event) => {
  ...
  if (request.url.includes("/api/")) {
    event.respondWith(
      networkFirstStrategy(request, CACHE_CONFIG.API_CACHE).catch(
        () =>
  ...
  if (
    ...
    event.respondWith(
      cacheFirstStrategy(request, CACHE_CONFIG.STATIC_CACHE).catch(async () => {
  ...
  if (request.destination === "image") {
    event.respondWith(
      staleWhileRevalidateStrategy(request, CACHE_CONFIG.IMAGE_CACHE).catch(
  ...
  event.respondWith(
    cacheFirstStrategy(request, CACHE_CONFIG.DYNAMIC_CACHE).catch(async () => {
      if (request.mode === "navigate") {
        return await caches.match(OFFLINE_FALLBACK_PAGE);
      }
    }),
